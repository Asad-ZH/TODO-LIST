...
$("#addingText").keypress(function(event){
    if(event.which === 13){
        ...
        $(this).val("");
        $(".UnChecked").append("<li class='selecting'><input type='checkbox'>" + todoText + "<i class='far fa-star'></i></li>");
		$('#addingText').blur();
		moved();
		moveBack();
		selecting();
...
//moving li's from one div to another
function moved(){
	var lis = document.querySelectorAll(".UnChecked li");
		...
		for (var i = 0; i < lis.length; i++){
			...
			var input = this.querySelector("input");
				...
				$(this).closest("li").prependTo(".Checked");
				selecting();
            }
        });
    }
...
function moveBack(){
	...
	for (var j = 0; j < lis2.length; j++){
			lis2[j].addEventListener("click",function(){
				...
				$(this).closest("li").prependTo(".UnChecked");
				selecting();
			}
		});
	}
...
//adding items on sidebar
  $("#closeForm1").click(function() {
	var header = $("#nameOfHeading").val();
	var email = $("#email").val();
	if(header !== ""){
	$(".sidenav").append("<li class='deleted'><i class='fas fa-list-ul'></i><a class='removingADeco' href='#'><span>" + header + "</span></a></li>");
	}
	if(email !== ""){
...
function selecting(){
	...
	//selecting and unselecting li's
	// $('.UnChecked li').on('click', function(){
	// 	$('.UnChecked li').css("background","white");
	// 	$('.Checked li').css("background"," rgba(255, 255, 255, 0.6)");
	// 	$(this).css("background","rgba(255, 255, 255, 0.8)");
	// });

	// $('.Checked li').on('click', function(){
	// 	$('.UnChecked li').css("background","white");
	// 	$('.Checked li').css("background"," rgba(255, 255, 255, 0.6)");
	// 	$(this).css("background","rgba(211, 211, 211, 0.397)");
	// });
